...
for ab_select in ['_a', '_b']:
    ...
    with open (fscript_fname, 'w') as fscript:
        ...
        with open('examples/mnist/lenet_solver_baseline_q2_drop.prototxt') as f:
            baseline_txt = f.read()
            ...
            for comb in hp:
                ...
                lines += '\nsnapshot_prefix: "examples/mnist/lenet_drop_{tmp_str}"\n'.format(**locals())


                fname = 'examples/mnist/lenet_drop_solver_%s.prototxt'%tmp_str

                with open(fname, 'w') as fproto:
                    fproto.write(lines + '\n' + baseline_txt)

                fscript.write(
                    '/local_stuff/caffe/build/tools/caffe train --solver={fname} >& log_drop_{tmp_str}.txt & \n'.format(**locals()))
